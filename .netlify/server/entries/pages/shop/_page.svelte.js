import { c as create_ssr_component, g as get_store_value, a as add_attribute, e as escape, h as each, v as validate_component } from "../../../chunks/ssr.js";
...
const ProductCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let cartItemIndex = cart.find((item) => item.id === products.id)?.quantity || 0;
  cart[cartItemIndex];
  ...
  return `<div class=""><div class="card-hover card flex m-5 p-5"><header><div class="card-image p-2"> <img${add_attribute("src", products.image_url, 0)} alt="KitchenAid mixer" style="max-width: 500px; max-height: 300px;"></div></header> <div class="card-content"><h3 class="card-title">${escape(products.name)}</h3> <p class="card-description">${escape(products.description)}</p> <p class="card-price">$${escape(products.price)}</p> <button class="btn variant-filled-surface m-4" data-svelte-h="svelte-j5i8gh">Add to Cart</button> <button class="btn variant-filled-surface" data-svelte-h="svelte-1kj3p5f">Remove from Cart</button></div></div></div>`;
});
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `  ${$$result.head += `<!-- HEAD_svelte-1i85ayg_START -->${$$result.title = `<title>Shop</title>`, ""}<meta name="description" content="Shop page"><!-- HEAD_svelte-1i85ayg_END -->`, ""} <h1 data-svelte-h="svelte-3yu6io">Shop</h1>  <div class="grid grid-cols-3">${each(products, (products2) => {
    return ` ${validate_component(ProductCard, "ProductCard").$$render($$result, { products: products2 }, {}, {})}`;
  })}</div>`;
});
export {
  Page as default
